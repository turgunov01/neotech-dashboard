  ...
  <div class="messages">
    <div class="message-container" v-if="loaded">
      <div class="message-incoming message-box">
        <mess :arr="messages" :type="'favourite'" />
        <div class="reader">
          <div class="reader-methods">
            <div class="reader-fn" @click="favourite">
              ...
              <p class="reader-fn-title">Удалить с избранных</p>
            </div>
          </div>
          <div class="reader-box">
            <div class="reader-header">
              <p class="reader-title">
                {{ area.reply_to.author }}
                {{ area.reply_to.author ? "/" : "" }} {{ area.reply_to.email }}
                {{ area.reply_to.email ? "/" : "" }} {{ area.reply_to.phone }}
                {{ area.reply_to.phone ? "/" : "" }}
              </p>
              <p class="reader-date">{{ area.reply_to.date }}</p>
            </div>
            <div class="reader-template" v-if="area.reply_to.message">
              {{ area.reply_to.message }}
            </div>
            <p>{{ area.author }}</p>
            <div class="reader-reply">
              <div class="reader-reply-box" v-html="area.message"></div>
            </div>
          </div>
        </div>
      </div>
...
const getList = async () => {
  ...
  await apiDataFetch(`/api/messages/`, "GET")
    ...
    .then(async (res) => {
      ...
      const current = await data.messages.find(
        (msg: any) => msg.id === Number(id)
      );

      const index = await data.messages.find(
        (msg: any) => msg.id == current.reply_to
      );

      current.reply_to = { ...index };
      area = { ...current };
      console.log(area);
...
const favourite = async () => {
  ...
  const index = messages.items.findIndex((item: any) => item.id === Number(id));
  const favourite = messages.items[index];

  area.type = favourite.prevType

  delete area.prevType
  delete area.isFav
  ...
  useRouter().push({ path: "/messages/outgoing/" });
};
